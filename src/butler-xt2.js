...
async function parseCuelistHtml(html) {
    ...
    cuelistRows.each((i, el) => {
        ...
        const cuelist = {
            index: i,
            name: columns[0].firstChild.data.split(" ").slice(1).join(" "),
...
async function parseSettings(html) {
    ...
    const settings = {
        ...
        uPnpEnabled: rows[11].firstChild.nextSibling.firstChild.attribs.value === "1",
        ...
        lockSettings: rows[21].firstChild.nextSibling.firstChild.attribs.value === "1",
        dmxBreakLength: Number(rows[22].firstChild.nextSibling.firstChild.attribs.value),
        dmxMabLength: Number(rows[23].firstChild.nextSibling.firstChild.attribs.value),
        rdmBreakLength: Number(rows[24].firstChild.nextSibling.firstChild.attribs.value),
        rdmMabLength: Number(rows[25].firstChild.nextSibling.firstChild.attribs.value),
...
ButlerXT2.prototype.fetchCuelistData = async function fetchCuelistData() {
    return await axios.get(`http://${this.host}/cldir.htm`)
...
ButlerXT2.prototype.fetchSettings = async function fetchCuelistData() {
    await axios.post(`http://${this.host}/setup.htm`, qs.stringify({ T30: 'ecue', BENTER: 'ENTER' }), {
        headers: {
...
ButlerXT2.prototype.playCuelist = async function playCuelist(cuelist) {
    ...
    await axios.post(`http://${this.host}/cldir.htm`, queryString, {
        headers: {
...
ButlerXT2.prototype.setCuelistLevel = async function setCuelistLevel(cuelist, level) {
